...
function gateplayer()
    local m = {
        ...
        agent = nil,
        conn = nil,
        key = math.random(1, 999999), -- math.random( 1, 999999999),
        lost_conn_time = nil,
        msgcache = {} -- 未发送的消息缓存
    }
    return m
...
s.rep.shutdown = function()
    skynet.error(s.name .. s.id .. " shutdown")
    closing = true
end

s.rep.send_by_fd = function(source, fd, msg)
...
s.rep.send = function(source, playerid, msg)
    ...
    s.rep.send_by_fd(nil, c.fd, msg)
end

s.rep.sure_agent = function(source, fd, playerid, agent)
...
s.rep.kick = function(source, playerid)
    local gplayer = players[playerid]
    if not gplayer then
        return
    end
...
local process_msg = function(fd, msgstr)
    ...
    if not playerid then
        ...
        if not agent then
            skynet.error("error haven't the agent")
            return
        end
