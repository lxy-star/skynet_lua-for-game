...
local balls = {} -- balls[playerid] = ball
local foods = {} -- foods[id] = food
local food_maxid = 0
local food_count = 0
...
service.rep.enter = function(source, playerid, node, agent)
    if balls[playerid] then
        skynet.error("error, already has the ball")
        return false
    end
    local b = ball()
    b.playerid = playerid
...
service.rep.leave = function(source, playerid)
...
service.rep.shift = function(source, playerid, x, y)
